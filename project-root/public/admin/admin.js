      ...
      .then(function (result) {
        ...
        (lessons || []).forEach(function (lesson) {
          ...
          row.innerHTML =
            ...
            '<td>' +
            '<button class="view-lesson-btn" data-id="' + (lesson._id || '') + '">View</button> ' +
        ...
        var viewBtns = document.querySelectorAll('.view-lesson-btn');
        for (var v = 0; v < viewBtns.length; v++) {
          viewBtns[v].addEventListener('click', function () {
            var id = this.getAttribute('data-id');
            if (id) {
          // Navigate to viewer page with ?id=<lessonId>
          window.location.href = '/lessons/lesson-viewer.html?id=' + id;
          }
        });
    }
      })
